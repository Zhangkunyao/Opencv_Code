    ...
    for part in sub_part:
        ...
        if len(x)!=0:
            ...
            if min(w, h) < 5:
                ...
                out[x,y,...] = sub_img[x-box_xmin,y-box_ymin]
        else:
            img_out = np.zeros((256, 256,3)).astype(np.uint8)
            IUV_map_out = np.zeros((256, 256,3)).astype(np.uint8)
            pose_out = np.zeros((256, 256,3)).astype(np.uint8)
